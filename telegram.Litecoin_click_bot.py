...
def logger(taskPerformed,money_link,view_time,profile,money='0',time2wait='0'):
    """ 
    ...
    csv_header = 'profile,Date,taskPerformed,money_link,view_time,money,time2wait\n'
    ...
    with open(logs_file, "r+",encoding="utf-8") as f:
        first_line = f.read()
        f.seek(0, 0)
        logs_write='['+profile+'] ,' + str(datetime.now())+ ',' + str(taskPerformed) + ', '+ str(money_link) + ','+str(view_time)+ 's,'+str(money)+ ','+str(time2wait)+'\n'
...
def message_bot(main_tab,profile):
    ...
    last_money_earned = "(//p[starts-with(text(),'You earned')])[last()]/strong"
        ...
        if status_of_ads=='NoMoreAds':
            ...
            try:
                if bot_link == get_href_from_popup(main_tab,ltc_message_bot_button,ltc_last_popup_link):
                    ...
                    toc = time.perf_counter()
                    logger('MessageBot',bot_channel_name,round(toc-tic,2),profile,money=get_text_of_element(main_tab,last_money_earned))
            except:
                toc = time.perf_counter()
                logger('MessageBot',bot_channel_name,round(toc-tic,2),profile,money=get_text_of_element(main_tab,last_money_earned))
...
def visit_website(main_tab,profile):
    ...
    while visit_website_stop:
        ...
        if status_of_ads=='NoMoreAds':
            ...
            close_browser_by_name()
            toc = time.perf_counter()
            logger(adsStatus,money_link,round(toc-tic,2),profile,money=get_text_of_element(main_tab,last_money_earned))
        ...
        # elif status_of_ads=='JoinChannel' or status_of_ads=='JoinGroup':
            ...
            try:
                if bot_link == get_href_from_popup(main_tab,ltc_last_join_channel_group_button,ltc_last_popup_link):
                    ...
                    logger('JoinChannel',channel_tag,round(toc-tic,2),profile,Join_channel_wait_time=str(time2wait))
...
def ppc_viewer(profile):
    ...
    try:
        join_channel(main_tab,profile,time2wait=6)
    except Exception as E:
        print(str(E))
